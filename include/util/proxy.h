...
struct recursive_reduction<R, O, I, Is...> : recursive_reduction<R, R<O, I>, Is...> {};
template<template<class, class> class R, class O, class... Is>
...
using instantiated_t =
    typename instantiated_traits<T, TL, std::make_index_sequence<std::tuple_size_v<TL>>, Args...>::
        type;
...
consteval bool has_copyability(constraint_level level) {
    ...
    case constraint_level::trivial:
        return std::is_trivially_copy_constructible_v<T> && std::is_trivially_destructible_v<T>;
    default:
        return false;
    }
}
...
consteval bool has_relocatability(constraint_level level) {
    ...
    case constraint_level::nothrow:
        return std::is_nothrow_move_constructible_v<T> && std::is_nothrow_destructible_v<T>;
    case constraint_level::trivial:
...
concept invocable_dispatch = (NE && std::is_nothrow_invocable_r_v<R, D, Args...>) ||
                             (!NE && std::is_invocable_r_v<R, D, Args...>);
template<class D, class P, qualifier_type Q, bool NE, class R, class... Args>
concept invocable_dispatch_ptr_indirect =
    ptr_traits<P, Q, NE>::applicable &&
    invocable_dispatch<D, NE, R, typename ptr_traits<P, Q, NE>::target_type, Args...>;
template<class D, class P, qualifier_type Q, bool NE, class R, class... Args>
concept invocable_dispatch_ptr_direct =
    invocable_dispatch<D, NE, R, add_qualifier_t<P, Q>, Args...> &&
    (Q != qualifier_type::rv || (NE && std::is_nothrow_destructible_v<P>) ||
     (!NE && std::is_destructible_v<P>));

template<bool NE, class R, class... Args>
using func_ptr_t = std::conditional_t<NE, R (*)(Args...) noexcept, R (*)(Args...)>;
...
R indirect_conv_dispatcher(add_qualifier_t<std::byte, Q> self, Args... args) noexcept(
    invocable_dispatch_ptr_indirect<D, P, Q, true, R, Args...>) {
    auto& qp = *std::launder(reinterpret_cast<add_qualifier_ptr_t<P, Q>>(&self));
    if constexpr (std::is_constructible_v<bool, decltype(qp)>) {
        assert(qp);
    }
...
template<class D, class P, qualifier_type Q, class R, class... Args>
R direct_conv_dispatcher(add_qualifier_t<std::byte, Q> self, Args... args) noexcept(
    invocable_dispatch_ptr_direct<D, P, Q, true, R, Args...>) {
    auto& qp = *std::launder(reinterpret_cast<add_qualifier_ptr_t<P, Q>>(&self));
...
R default_conv_dispatcher(add_qualifier_t<std::byte, Q>, Args... args) noexcept(
    invocable_dispatch<D, true, R, std::nullptr_t, Args...>) {
    return invoke_dispatch<D, R>(nullptr, std::forward<Args>(args)...);
}
template<class P>
void copying_dispatcher(std::byte& self, const std::byte& rhs) noexcept(
    has_copyability<P>(constraint_level::nothrow)) {
    std::construct_at(reinterpret_cast<P*>(&self), *std::launder(reinterpret_cast<const P*>(&rhs)));
}
template<std::size_t Len, std::size_t Align>
void copying_default_dispatcher(std::byte& self, const std::byte& rhs) noexcept {
    std::uninitialized_copy_n(std::assume_aligned<Align>(&rhs), Len,
                              std::assume_aligned<Align>(&self));
}
template<class P>
void relocation_dispatcher(std::byte& self, const std::byte& rhs) noexcept(
...
template<class P>
void destruction_dispatcher(std::byte& self) noexcept(
...
struct overload_traits_impl : applicable_traits {
    ...
    struct meta_provider {
        template<class P>
        static consteval auto get() -> func_ptr_t<NE, R, add_qualifier_t<std::byte, Q>, Args...> {
            if constexpr (!IsDirect && invocable_dispatch_ptr_indirect<D, P, Q, NE, R, Args...>) {
...
struct overload_traits<R(Args...)> : overload_traits_impl<qualifier_type::lv, false, R, Args...> {};
template<class R, class... Args>
...
template<template<class> class O>
struct overload_substitution_traits<facade_aware_overload_t<O>> : applicable_traits {
    template<class F>
    using type = O<F>;
};
template<class O, class F>
using substituted_overload_t = typename overload_substitution_traits<O>::template type<F>;
template<class O>
concept extended_overload =
    overload_traits<O>::applicable || overload_substitution_traits<O>::applicable;
...
consteval bool diagnose_proxiable_required_convention_not_implemented() {
    constexpr bool verdict =
        ...
        overload_traits<substituted_overload_t<O, F>>::template is_applicable_ptr<IsDirect, D, P>();
    static_assert(verdict, "not proxiable due to a required convention not implemented");
    return verdict;
}
...
template<class... Ms>
using composite_meta = recursive_reduction_t<meta_reduction_t, composite_meta_impl<>, Ms...>;
...
template<class C>
    ...
    } && is_is_direct_well_formed<C>() && std::is_trivial_v<typename C::dispatch_type> &&
...
struct conv_traits_impl<C, F, Os...> : applicable_traits {
    using meta = composite_meta_impl<
        dispatcher_meta<typename overload_traits<substituted_overload_t<Os, F>>::
    ...
    static consteval bool diagnose_proxiable() {
        bool verdict = true;
        ((verdict &=
          diagnose_proxiable_required_convention_not_implemented<P, F, C::is_direct,
    ...
    static constexpr bool applicable_ptr =
        (overload_traits<substituted_overload_t<Os, F>>::
             template is_applicable_ptr<C::is_direct, typename C::dispatch_type, P>() &&
         ...);
};
template<class C, class F>
struct conv_traits : instantiated_t<conv_traits_impl, typename C::overload_types, C, F> {};
...
struct refl_meta {
    template<class P>
        requires(IsDirect)
    constexpr explicit refl_meta(std::in_place_type_t<P>) : reflector(std::in_place_type<P>) {
    }
    template<class P>
        requires(!IsDirect)
...
template<class R>
    requires(requires { typename R::reflector_type; } && is_is_direct_well_formed<R>())
...
consteval bool diagnose_proxiable_required_reflection_not_implemented() {
    ...
    static_assert(verdict, "not proxiable due to a required reflection not implemented");
    return verdict;
}
...
struct refl_traits {
    ...
    static consteval bool diagnose_proxiable() {
        return diagnose_proxiable_required_reflection_not_implemented<P, F, R::is_direct,
...
class ___PRO_ENFORCE_EBO composite_accessor_impl : public As... {
    ...
    template<facade>
    friend struct pro::proxy_indirect_accessor;

    composite_accessor_impl() noexcept                                          = default;
    composite_accessor_impl(const composite_accessor_impl&) noexcept            = default;
    composite_accessor_impl& operator=(const composite_accessor_impl&) noexcept = default;
};

template<class T>
struct accessor_traits_impl : std::type_identity<void> {};
template<class T>
    requires(std::is_nothrow_default_constructible_v<T> && std::is_trivially_copyable_v<T> &&
             !std::is_final_v<T>)
...
struct composite_accessor_reduction<IsDirect, F, composite_accessor_impl<As...>, I>
...
struct composite_accessor_helper {
    template<class O, class I>
    using reduction_t = typename composite_accessor_reduction<IsDirect, F, O, I>::type;
};
template<bool IsDirect, class F, class... Ts>
using composite_accessor =
    recursive_reduction_t<composite_accessor_helper<IsDirect, F>::template reduction_t,
...
template<class A1, class A2>
using merged_composite_accessor = typename composite_accessor_merge_traits<A1, A2>::type;
...
template<class P>
struct proxy_traits : inapplicable_traits {};
template<class F>
struct proxy_traits<proxy<F>> : applicable_traits {
    using facade_type = F;
};
...
template<class F>
    requires(
        ...
        instantiated_t<basic_facade_conv_traits_impl, typename F::convention_types>::applicable &&
        is_tuple_like_well_formed<typename F::reflection_types>() &&
        instantiated_t<basic_facade_refl_traits_impl, typename F::reflection_types>::applicable)
...
struct facade_conv_traits_impl<F, Cs...> : applicable_traits {
    ...
    static constexpr bool is_invocable = std::is_base_of_v<
        dispatcher_meta<typename overload_traits<O>::template meta_provider<IsDirect, D>>,
        conv_meta>;
};
...
template<class F>
    requires(instantiated_t<facade_conv_traits_impl, typename F::convention_types, F>::applicable)
struct facade_traits<F> : instantiated_t<facade_conv_traits_impl, typename F::convention_types, F>,
                          instantiated_t<facade_refl_traits_impl, typename F::reflection_types, F> {
    using copyability_meta = lifetime_meta_t<copyability_meta_provider, F::constraints.copyability>;
    using relocatability_meta =
        lifetime_meta_t<relocatability_meta_provider,
                        F::constraints.copyability == constraint_level::trivial
                            ? constraint_level::trivial
                            : F::constraints.relocatability>;
    using destructibility_meta =
        lifetime_meta_t<destructibility_meta_provider, F::constraints.destructibility>;
    ...
    using direct_accessor = merged_composite_accessor<typename facade_traits::conv_direct_accessor,
                                                      typename facade_traits::refl_direct_accessor>;

    template<class P>
    static consteval void diagnose_proxiable() {
        bool verdict = true;
        verdict &= diagnose_proxiable_size_too_large<P, F, sizeof(P), F::constraints.max_size>();
        verdict &= diagnose_proxiable_align_too_large<P, F, alignof(P), F::constraints.max_align>();
        verdict &= diagnose_proxiable_insufficient_copyability<P, F, F::constraints.copyability>();
        verdict &=
            diagnose_proxiable_insufficient_relocatability<P, F, F::constraints.relocatability>();
        verdict &=
            diagnose_proxiable_insufficient_destructibility<P, F, F::constraints.destructibility>();
    ...
    template<class P>
    static constexpr bool applicable_ptr =
        sizeof(P) <= F::constraints.max_size && alignof(P) <= F::constraints.max_align &&
...
struct meta_ptr_direct_impl : private M {
    using M::M;
    bool has_value() const noexcept {
        return this->DM::dispatcher != nullptr;
    }
    void reset() noexcept {
        this->DM::dispatcher = nullptr;
    }
    const M* operator->() const noexcept {
        return this;
    }
...
struct meta_ptr_traits_impl<composite_meta_impl<dispatcher_meta<MP>, Ms...>>
    : std::type_identity<meta_ptr_direct_impl<composite_meta_impl<dispatcher_meta<MP>, Ms...>,
                                              dispatcher_meta<MP>>> {};
template<class M>
struct meta_ptr_traits : std::type_identity<meta_ptr_indirect_impl<M>> {};
template<class M>
    requires(sizeof(M) <= sizeof(ptr_prototype) && alignof(M) <= alignof(ptr_prototype) &&
             std::is_nothrow_default_constructible_v<M> && std::is_trivially_copyable_v<M>)
        ...
        auto dispatcher =
            get_meta(p)
                .template dispatcher_meta<
                    typename overload_traits<O>::template meta_provider<IsDirect, D>>::dispatcher;
        if constexpr (IsDirect && overload_traits<O>::qualifier == qualifier_type::rv) {
...
struct proxy_indirect_accessor {};
template<facade F>
    requires(!std::is_same_v<typename details::facade_traits<F>::indirect_accessor,
                             details::composite_accessor_impl<>>)
struct proxy_indirect_accessor<F> : details::facade_traits<F>::indirect_accessor {};
...
class proxy : public details::facade_traits<F>::direct_accessor {
  ...
  public:
    proxy() noexcept {
        ___PRO_DEBUG(
            std::ignore =
                static_cast<proxy_indirect_accessor<F>* (proxy::*)() noexcept>(&proxy::operator->);
            std::ignore =
                static_cast<const proxy_indirect_accessor<F>* (proxy::*)() const noexcept>(
                    &proxy::operator->);
            std::ignore =
                static_cast<proxy_indirect_accessor<F>& (proxy::*)() & noexcept>(&proxy::operator*);
            std::ignore =
    ...
    = default;
    proxy(const proxy& rhs) noexcept(F::constraints.copyability == constraint_level::nothrow)
    ...
    proxy(proxy&& rhs) noexcept(F::constraints.relocatability == constraint_level::nothrow)
    ...
    template<class P>
    constexpr proxy(P&& ptr) noexcept(std::is_nothrow_constructible_v<std::decay_t<P>, P>)
    ...
    constexpr explicit proxy(std::in_place_type_t<P>,
                             Args&&... args) noexcept(std::is_nothrow_constructible_v<P, Args...>)
    ...
    constexpr explicit proxy(
        std::in_place_type_t<P>,
        std::initializer_list<U> il,
        Args&&... args) noexcept(std::is_nothrow_constructible_v<P,
    ...
    proxy&
    operator=(proxy&& rhs) noexcept(F::constraints.relocatability >= constraint_level::nothrow &&
    ...
    template<class P>
    constexpr proxy&
    operator=(P&& ptr) noexcept(std::is_nothrow_constructible_v<std::decay_t<P>, P> &&
                                F::constraints.destructibility >= constraint_level::nothrow)
        requires(!details::proxy_traits<std::decay_t<P>>::applicable &&
                 std::is_constructible_v<std::decay_t<P>, P> &&
                 F::constraints.destructibility >= constraint_level::nontrivial)
    ...
    void reset() noexcept(F::constraints.destructibility >= constraint_level::nothrow)
        requires(F::constraints.destructibility >= constraint_level::nontrivial)
    {
        std::destroy_at(this);
        meta_.reset();
    }
    void swap(proxy& rhs) noexcept(F::constraints.relocatability >= constraint_level::nothrow ||
                                   F::constraints.copyability == constraint_level::trivial)
        requires(F::constraints.relocatability >= constraint_level::nontrivial ||
                 F::constraints.copyability == constraint_level::trivial)
    {
    ...
    template<class P, class... Args>
    constexpr P& emplace(Args&&... args) noexcept(std::is_nothrow_constructible_v<P, Args...> &&
                                                  F::constraints.destructibility >=
                                                      constraint_level::nothrow)
        requires(std::is_constructible_v<P, Args...> &&
                 F::constraints.destructibility >= constraint_level::nontrivial)
  ...
  private:
    ...
    constexpr P& initialize(Args&&... args) {
        P& result = *std::construct_at(reinterpret_cast<P*>(ptr_), std::forward<Args>(args)...);
...
auto proxy_invoke(proxy<F>& p, Args&&... args) ->
    ...
    return details::proxy_helper<F>::template invoke<IsDirect, D, O, details::qualifier_type::lv>(
...
auto proxy_invoke(const proxy<F>& p, Args&&... args) ->
    ...
    return details::proxy_helper<F>::template invoke<IsDirect, D, O,
                                                     details::qualifier_type::const_lv>(
        p, std::forward<Args>(args)...);
}
template<bool IsDirect, class D, class O, facade F, class... Args>
auto proxy_invoke(proxy<F>&& p, Args&&... args) ->
    typename details::overload_traits<O>::return_type {
    return details::proxy_helper<F>::template invoke<IsDirect, D, O, details::qualifier_type::rv>(
...
auto proxy_invoke(const proxy<F>&& p, Args&&... args) ->
    typename details::overload_traits<O>::return_type {
    return details::proxy_helper<F>::template invoke<IsDirect, D, O,
                                                     details::qualifier_type::const_rv>(
        std::move(p), std::forward<Args>(args)...);
}
...
proxy<F>& access_proxy(A& a) noexcept {
    return details::proxy_helper<F>::template access<A, details::qualifier_type::lv>(a);
}
template<facade F, class A>
const proxy<F>& access_proxy(const A& a) noexcept {
    return details::proxy_helper<F>::template access<A, details::qualifier_type::const_lv>(a);
}
template<facade F, class A>
proxy<F>&& access_proxy(A&& a) noexcept {
    return details::proxy_helper<F>::template access<A, details::qualifier_type::rv>(
        std::forward<A>(a));
}
template<facade F, class A>
const proxy<F>&& access_proxy(const A&& a) noexcept {
    return details::proxy_helper<F>::template access<A, details::qualifier_type::const_rv>(
...
class inplace_ptr {
  public:
    template<class... Args>
    inplace_ptr(std::in_place_t, Args&&... args) : value_(std::forward<Args>(args)...) {
    }
    inplace_ptr(const inplace_ptr&) noexcept(std::is_nothrow_copy_constructible_v<T>) = default;
...
class allocated_ptr {
  public:
    ...
    allocated_ptr(const allocated_ptr& rhs)
        ...
        : alloc_(rhs.alloc_),
          ptr_(rhs.ptr_ == nullptr ? nullptr : allocate<T>(alloc_, std::as_const(*rhs.ptr_))) {
    }
    allocated_ptr(allocated_ptr&& rhs) noexcept(std::is_nothrow_move_constructible_v<Alloc>)
  ...
  public:
    ...
    compact_ptr(const compact_ptr& rhs)
        ...
        : ptr_(rhs.ptr_ == nullptr ? nullptr
                                   : allocate<storage>(rhs.ptr_->alloc,
                                                       rhs.ptr_->alloc,
                                                       std::as_const(rhs.ptr_->value))) {
    }
    compact_ptr(compact_ptr&& rhs) noexcept : ptr_(std::exchange(rhs.ptr_, nullptr)) {
    }
    ~compact_ptr() {
        if (ptr_ != nullptr) {
    ...
    T&& operator*() && noexcept {
        return std::forward<T>(ptr_->value);
    }
    const T&& operator*() const&& noexcept {
...
constexpr proxy<F> make_proxy_impl(Args&&... args) {
    if constexpr (proxiable<inplace_ptr<T>, F>) {
        ...
        return allocate_proxy_impl<F, T>(std::allocator<T>{}, std::forward<Args>(args)...);
    }
}
#endif // __STDC_HOSTED__
...
constexpr proxy<F>
make_proxy_inplace(Args&&... args) noexcept(std::is_nothrow_constructible_v<T, Args...>)
...
constexpr proxy<F> make_proxy_inplace(std::initializer_list<U> il, Args&&... args) noexcept(
    std::is_nothrow_constructible_v<T, std::initializer_list<U>&, Args...>)
    requires(std::is_constructible_v<T, std::initializer_list<U>&, Args...>)
{
    return proxy<F>{std::in_place_type<details::inplace_ptr<T>>, std::in_place, il,
                    std::forward<Args>(args)...};
}
template<facade F, class T>
constexpr proxy<F>
make_proxy_inplace(T&& value) noexcept(std::is_nothrow_constructible_v<std::decay_t<T>, T>)
    requires(std::is_constructible_v<std::decay_t<T>, T>)
{
    return proxy<F>{std::in_place_type<details::inplace_ptr<std::decay_t<T>>>, std::in_place,
                    std::forward<T>(value)};
}
...
concept proxiable_target =
    inplace_proxiable_target<T, F> || proxiable<details::allocated_ptr<T, std::allocator<void>>, F>;
...
constexpr proxy<F> allocate_proxy(const Alloc& alloc, T&& value)
    requires(std::is_constructible_v<std::decay_t<T>, T>)
{
    return details::allocate_proxy_impl<F, std::decay_t<T>>(alloc, std::forward<T>(value));
}
template<facade F, class T, class... Args>
...
#define ___PRO_DEF_MEM_ACCESSOR_TEMPLATE(__MACRO, ...)                                             \
    ...
    struct ___PRO_ENFORCE_EBO accessor {                                                           \
        accessor() = delete;                                                                       \
    };                                                                                             \
    template<class __F, bool __IsDirect, class __D, class... __Os>                                 \
        requires(sizeof...(__Os) > 1u &&                                                           \
                 (::std::is_constructible_v<accessor<__F, __IsDirect, __D, __Os>> && ...))         \
    struct accessor<__F, __IsDirect, __D, __Os...> : accessor<__F, __IsDirect, __D, __Os>... {     \
    ...
    __MACRO(&& noexcept, ::pro::access_proxy<__F>(::std::move(*this)), __VA_ARGS__);               \
    __MACRO(const, ::pro::access_proxy<__F>(*this), __VA_ARGS__);                                  \
    __MACRO(const noexcept, ::pro::access_proxy<__F>(*this), __VA_ARGS__);                         \
    __MACRO(const&, ::pro::access_proxy<__F>(*this), __VA_ARGS__);                                 \
    __MACRO(const& noexcept, ::pro::access_proxy<__F>(*this), __VA_ARGS__);                        \
    __MACRO(const&&, ::pro::access_proxy<__F>(::std::move(*this)), __VA_ARGS__);                   \
    __MACRO(const&& noexcept, ::pro::access_proxy<__F>(::std::move(*this)), __VA_ARGS__);
...
#define ___PRO_DEF_FREE_ACCESSOR_TEMPLATE(__MACRO, ...)                                            \
    ...
    template<class __F, bool __IsDirect, class __D, class... __Os>                                 \
        requires(sizeof...(__Os) > 1u &&                                                           \
                 (::std::is_constructible_v<accessor<__F, __IsDirect, __D, __Os>> && ...))         \
    struct accessor<__F, __IsDirect, __D, __Os...> : accessor<__F, __IsDirect, __D, __Os>... {};   \
    __MACRO(, , ___PRO_ADL_ARG& __self, ::pro::access_proxy<__F>(__self), __VA_ARGS__);            \
    __MACRO(noexcept, noexcept, ___PRO_ADL_ARG& __self, ::pro::access_proxy<__F>(__self),          \
            __VA_ARGS__);                                                                          \
    __MACRO(&, , ___PRO_ADL_ARG& __self, ::pro::access_proxy<__F>(__self), __VA_ARGS__);           \
    __MACRO(& noexcept, noexcept, ___PRO_ADL_ARG& __self, ::pro::access_proxy<__F>(__self),        \
            __VA_ARGS__);                                                                          \
    __MACRO(&&, , ___PRO_ADL_ARG&& __self,                                                         \
            ::pro::access_proxy<__F>(::std::forward<decltype(__self)>(__self)), __VA_ARGS__);      \
    __MACRO(&& noexcept, noexcept, ___PRO_ADL_ARG&& __self,                                        \
            ::pro::access_proxy<__F>(::std::forward<decltype(__self)>(__self)), __VA_ARGS__);      \
    __MACRO(const, , const ___PRO_ADL_ARG& __self, ::pro::access_proxy<__F>(__self), __VA_ARGS__); \
    __MACRO(const noexcept, noexcept, const ___PRO_ADL_ARG& __self,                                \
            ::pro::access_proxy<__F>(__self), __VA_ARGS__);                                        \
    __MACRO(const&, , const ___PRO_ADL_ARG& __self, ::pro::access_proxy<__F>(__self),              \
            __VA_ARGS__);                                                                          \
    __MACRO(const& noexcept, noexcept, const ___PRO_ADL_ARG& __self,                               \
            ::pro::access_proxy<__F>(__self), __VA_ARGS__);                                        \
...
#define ___PRO_GEN_DEBUG_SYMBOL_FOR_MEM_ACCESSOR(...)                                              \
    ___PRO_DEBUG(accessor() noexcept { ::std::ignore = &accessor::__VA_ARGS__; })

#ifdef __cpp_rtti
class bad_proxy_cast : public std::bad_cast {
  public:
    char const* what() const noexcept override {
        return "pro::bad_proxy_cast";
    }
};
#endif // __cpp_rtti
...
using adl_accessor_arg_t = std::conditional_t<IsDirect, proxy<F>, proxy_indirect_accessor<F>>;
...
struct cast_dispatch_base {
    ___PRO_DEF_MEM_ACCESSOR_TEMPLATE(
        ___PRO_DEF_CAST_ACCESSOR, operator typename overload_traits<__Os>::return_type)
};
#undef ___PRO_DEF_CAST_ACCESSOR

struct upward_conversion_dispatch : cast_dispatch_base<false, true> {
    template<class T>
    T&& operator()(T&& self) const noexcept {
...
struct explicit_conversion_adapter {
    explicit explicit_conversion_adapter(T&& value) noexcept : value_(std::forward<T>(value)) {
    }
...
consteval auto make_copyable(proxiable_ptr_constraints value, constraint_level cl) {
    if (value.copyability < cl) {
        value.copyability = cl;
    }
    return value;
}
consteval auto make_relocatable(proxiable_ptr_constraints value, constraint_level cl) {
    if (value.relocatability < cl) {
        value.relocatability = cl;
    }
    return value;
}
consteval auto make_destructible(proxiable_ptr_constraints value, constraint_level cl) {
    if (value.destructibility < cl) {
        value.destructibility = cl;
    }
    return value;
}
consteval auto merge_constraints(proxiable_ptr_constraints a, proxiable_ptr_constraints b) {
...
struct accessor_instantiation_traits<
    std::void_t<typename T::template accessor<F, IsDirect, T, Args...>>,
    T,
    F,
    IsDirect,
    Args...> : std::type_identity<typename T::template accessor<F, IsDirect, T, Args...>> {};
template<class T, class F, bool IsDirect, class... Args>
...
struct conv_impl {
    ...
    template<class F>
    using accessor = instantiated_accessor_t<D, F, IsDirect, substituted_overload_t<Os, F>...>;
};
template<bool IsDirect, class R>
struct refl_impl {
    static constexpr bool is_direct = IsDirect;
...
using merge_conv_t =
    instantiated_t<merge_conv_traits<C1::is_direct, typename C1::dispatch_type>::template type,
...
template<class... Cs1, class C2, class... Cs3, class C>
    requires(C::is_direct == C2::is_direct &&
...
struct add_conv_reduction<std::tuple<Cs...>, std::tuple<>, C>
    : std::type_identity<
          std::tuple<Cs..., merge_conv_t<conv_impl<C::is_direct, typename C::dispatch_type>, C>>> {
};
template<class Cs, class C>
using add_conv_t = typename add_conv_reduction<std::tuple<>, Cs, C>::type;

template<class F, constraint_level CL>
using copy_conversion_overload = proxy<F>() const& noexcept(CL >= constraint_level::nothrow);
template<class F, constraint_level CL>
using move_conversion_overload = proxy<F>() && noexcept(CL >= constraint_level::nothrow);
template<class Cs, class F, constraint_level CCL, constraint_level RCL>
struct add_upward_conversion_conv
    : std::type_identity<add_conv_t<Cs,
                                    conv_impl<true,
...
struct add_upward_conversion_conv<Cs, F, constraint_level::none, RCL>
    : std::type_identity<add_conv_t<
          Cs,
          conv_impl<true, upward_conversion_dispatch, move_conversion_overload<F, RCL>>>> {};
template<class Cs, class F, constraint_level CCL>
struct add_upward_conversion_conv<Cs, F, CCL, constraint_level::none>
    : std::type_identity<add_conv_t<
          Cs,
          conv_impl<true, upward_conversion_dispatch, copy_conversion_overload<F, CCL>>>> {};
template<class Cs, class F>
struct add_upward_conversion_conv<Cs, F, constraint_level::none, constraint_level::none>
...
using merge_facade_conv_t = typename add_upward_conversion_conv<
    ...
    (WithUpwardConversion && F::constraints.copyability != constraint_level::trivial)
        ? F::constraints.relocatability
...
struct observer_ptr {
  public:
    ...
    auto operator->() noexcept {
        return std::addressof(static_cast<LR>(*ptr_));
    }
    auto operator->() const noexcept {
        return std::addressof(static_cast<CLR>(*ptr_));
    }
    LR operator*() & noexcept {
        return static_cast<LR>(*ptr_);
    }
    CLR operator*() const& noexcept {
        return static_cast<CLR>(*ptr_);
    }
    RR operator*() && noexcept {
        return static_cast<RR>(*ptr_);
    }
    CRR operator*() const&& noexcept {
        return static_cast<CRR>(*ptr_);
    }
...
struct proxy_view_dispatch : cast_dispatch_base<false, true> {
        ...
        })
    {
        return observer_ptr<decltype(*value), decltype(*std::as_const(value)),
                            ...
                            decltype(*std::move(std::as_const(value)))>{std::addressof(*value)};
    }
};

template<class O>
using observer_upward_conversion_overload =
    proxy_view<typename proxy_traits<typename overload_traits<O>::return_type>::facade_type>()
        const noexcept;
...
struct observer_upward_conversion_conv_reduction<conv_impl<true, upward_conversion_dispatch, Os...>,
                                                 O>
    : std::type_identity<conv_impl<true,
                                   upward_conversion_dispatch,
                                   Os...,
...
using observer_upward_conversion_conv =
    recursive_reduction_t<observer_upward_conversion_conv_reduction_t,
                          conv_impl<true, upward_conversion_dispatch>,
                          Os...>;

template<class D, class F, class... Os>
using observer_indirect_conv = conv_impl<false, D, substituted_overload_t<Os, F>...>;

template<class C, class F>
struct observer_conv_traits : inapplicable_traits {};
template<class C, class F>
    requires(C::is_direct && std::is_same_v<typename C::dispatch_type, upward_conversion_dispatch>)
struct observer_conv_traits<C, F>
    : applicable_traits,
      std::type_identity<
          instantiated_t<observer_upward_conversion_conv, typename C::overload_types>> {};
template<class C, class F>
    requires(!C::is_direct)
struct observer_conv_traits<C, F> : applicable_traits,
                                    std::type_identity<instantiated_t<observer_indirect_conv,
                                                                      typename C::overload_types,
                                                                      typename C::dispatch_type,
                                                                      F>> {};
...
struct observer_conv_reduction<F, std::tuple<Cs...>, I>
    : std::type_identity<std::tuple<Cs..., typename observer_conv_traits<I, F>::type>> {};
template<class F>
...
struct observer_refl_reduction<std::tuple<Rs...>, R> : std::type_identity<std::tuple<Rs..., R>> {};
template<class O, class I>
using observer_refl_reduction_t = typename observer_refl_reduction<O, I>::type;

template<class F, class... Cs>
struct observer_facade_conv_impl {
    using convention_types = recursive_reduction_t<observer_conv_reduction_helper<F>::template type,
                                                   std::tuple<>,
                                                   Cs...>;
};
template<class... Rs>
struct observer_facade_refl_impl {
    using reflection_types = recursive_reduction_t<observer_refl_reduction_t, std::tuple<>, Rs...>;
};
...
template<>
struct format_overload_traits<char> : std::type_identity<std::format_context::iterator(
                                          std::string_view spec, std::format_context& fc) const> {};
template<>
struct format_overload_traits<wchar_t>
    : std::type_identity<std::wformat_context::iterator(std::wstring_view     spec,
                                                        std::wformat_context& fc) const> {};
template<class CharT>
...
struct proxy_cast_accessor_impl {
    using _Self = add_qualifier_t<adl_accessor_arg_t<F, IsDirect>, overload_traits<O>::qualifier>;
    template<class T>
    friend T proxy_cast(_Self self) {
        ...
        if constexpr (std::is_lvalue_reference_v<T>) {
            ...
            proxy_cast_context ctx{.type_ptr   = &typeid(T),
                                   ...
                                   .result_ptr = &result};
            proxy_invoke<IsDirect, D, O>(access_proxy<F>(std::forward<_Self>(self)), ctx);
            if (result == nullptr) {
                ___PRO_THROW(bad_proxy_cast{});
            }
            ...
            proxy_cast_context                    ctx{
                                   .type_ptr = &typeid(T), .is_ref = false, .is_const = false, .result_ptr = &result};
            proxy_invoke<IsDirect, D, O>(access_proxy<F>(std::forward<_Self>(self)), ctx);
            if (!result.has_value()) {
                ___PRO_THROW(bad_proxy_cast{});
            }
...
struct proxy_typeid_reflector {
    template<class T>
    constexpr explicit proxy_typeid_reflector(std::in_place_type_t<T>) : info(&typeid(T)) {
    }
...
struct basic_facade_builder {
    ...
    using add_indirect_convention =
        basic_facade_builder<details::add_conv_t<Cs, details::conv_impl<false, D, Os...>>, Rs, C>;
    template<class D, details::extended_overload... Os>
        requires(sizeof...(Os) > 0u)
    using add_direct_convention =
        basic_facade_builder<details::add_conv_t<Cs, details::conv_impl<true, D, Os...>>, Rs, C>;
    template<class D, details::extended_overload... Os>
        requires(sizeof...(Os) > 0u)
    using add_convention = add_indirect_convention<D, Os...>;
    template<class R>
    using add_indirect_reflection =
        basic_facade_builder<Cs, details::add_tuple_t<Rs, details::refl_impl<false, R>>, C>;
    template<class R>
    using add_direct_reflection =
        basic_facade_builder<Cs, details::add_tuple_t<Rs, details::refl_impl<true, R>>, C>;
    template<class R>
    using add_reflection = add_indirect_reflection<R>;
    template<facade F, bool WithUpwardConversion = false>
    using add_facade =
        basic_facade_builder<details::merge_facade_conv_t<Cs, F, WithUpwardConversion>,
    ...
    using restrict_layout =
        basic_facade_builder<Cs, Rs, details::make_restricted_layout(C, PtrSize, PtrAlign)>;
    template<constraint_level CL>
    using support_copy = basic_facade_builder<Cs, Rs, details::make_copyable(C, CL)>;
    template<constraint_level CL>
    using support_relocation = basic_facade_builder<Cs, Rs, details::make_relocatable(C, CL)>;
    template<constraint_level CL>
    using support_destruction = basic_facade_builder<Cs, Rs, details::make_destructible(C, CL)>;
#if __STDC_HOSTED__
    using support_format =
        add_convention<details::format_dispatch, details::format_overload_t<char>>;
    using support_wformat =
        add_convention<details::format_dispatch, details::format_overload_t<wchar_t>>;
#endif // __STDC_HOSTED__
#ifdef __cpp_rtti
    using support_indirect_rtti = basic_facade_builder<
        details::add_conv_t<Cs,
                            details::conv_impl<false,
                                               details::proxy_cast_dispatch,
                                               void(details::proxy_cast_context) &,
                                               void(details::proxy_cast_context) const&,
                                               void(details::proxy_cast_context) &&>>,
        details::add_tuple_t<Rs, details::refl_impl<false, details::proxy_typeid_reflector>>,
        C>;
    using support_direct_rtti = basic_facade_builder<
        details::add_conv_t<Cs,
                            details::conv_impl<true,
                                               details::proxy_cast_dispatch,
                                               void(details::proxy_cast_context) &,
                                               void(details::proxy_cast_context) const&,
                                               void(details::proxy_cast_context) &&>>,
        details::add_tuple_t<Rs, details::refl_impl<true, details::proxy_typeid_reflector>>,
        C>;
    using support_rtti = support_indirect_rtti;
#endif // __cpp_rtti
    using support_view =
        add_direct_convention<details::proxy_view_dispatch,
...
template<facade F>
struct observer_facade
    : details::instantiated_t<details::observer_facade_conv_impl, typename F::convention_types, F>,
      details::instantiated_t<details::observer_facade_refl_impl, typename F::reflection_types> {
...
using facade_builder =
    basic_facade_builder<std::tuple<>,
                         std::tuple<>,
                         proxiable_ptr_constraints{.max_size        = details::invalid_size,
...
#define ___PRO_DEF_LHS_LEFT_OP_ACCESSOR(Q, SELF, ...)                                              \
    ...
    struct accessor<__F, __IsDirect, __D, R() Q> {                                                 \
        ...
        R __VA_ARGS__() Q {                                                                        \
            return proxy_invoke<__IsDirect, __D, R() Q>(SELF);                                     \
        }                                                                                          \
    }
#define ___PRO_DEF_LHS_ANY_OP_ACCESSOR(Q, SELF, ...)                                               \
    ...
    struct accessor<__F, __IsDirect, __D, R(Args...) Q> {                                          \
        ...
        R __VA_ARGS__(Args... args) Q {                                                            \
            return proxy_invoke<__IsDirect, __D, R(Args...) Q>(SELF, std::forward<Args>(args)...); \
        }                                                                                          \
    }
...
#define ___PRO_LHS_LEFT_OP_DISPATCH_BODY_IMPL(...)                                                 \
    ...
    decltype(auto) operator()(T&& self)                                                            \
        const ___PRO_DIRECT_FUNC_IMPL(__VA_ARGS__ std::forward<T>(self))
#define ___PRO_LHS_UNARY_OP_DISPATCH_BODY_IMPL(...)                                                \
    template<class T>                                                                              \
    decltype(auto) operator()(T&& self)                                                            \
        const ___PRO_DIRECT_FUNC_IMPL(__VA_ARGS__ std::forward<T>(self)) template<class T>         \
        decltype(auto) operator()(T&& self, int)                                                   \
            const ___PRO_DIRECT_FUNC_IMPL(std::forward<T>(self) __VA_ARGS__)
#define ___PRO_LHS_BINARY_OP_DISPATCH_BODY_IMPL(...)                                               \
    template<class T, class Arg>                                                                   \
    decltype(auto) operator()(T&& self, Arg&& arg)                                                 \
        const ___PRO_DIRECT_FUNC_IMPL(std::forward<T>(self) __VA_ARGS__ std::forward<Arg>(arg))
...
#define ___PRO_LHS_OP_DISPATCH_IMPL(TYPE, ...)                                                     \
    ...
    struct operator_dispatch<#__VA_ARGS__, false> {                                                \
        ___PRO_LHS_##TYPE##_OP_DISPATCH_BODY_IMPL(__VA_ARGS__) ___PRO_DEF_MEM_ACCESSOR_TEMPLATE(   \
...
#define ___PRO_DEF_RHS_OP_ACCESSOR(Q, NE, SELF_ARG, SELF, ...)                                     \
    ...
    struct accessor<__F, __IsDirect, __D, R(Arg) Q> {                                              \
        friend R operator __VA_ARGS__(Arg arg, SELF_ARG) NE {                                      \
            return proxy_invoke<__IsDirect, __D, R(Arg) Q>(SELF, std::forward<Arg>(arg));          \
        }                                                                                          \
        ___PRO_DEBUG( \
      accessor() noexcept { std::ignore = &_symbol_guard; } \
    \
...
#define ___PRO_RHS_OP_DISPATCH_IMPL(...)                                                           \
    ...
    struct operator_dispatch<#__VA_ARGS__, true> {                                                 \
        ...
        decltype(auto) operator()(T&& self, Arg&& arg) const                                       \
            ___PRO_DIRECT_FUNC_IMPL(std::forward<Arg>(arg) __VA_ARGS__ std::forward<T>(self))      \
                ___PRO_DEF_FREE_ACCESSOR_TEMPLATE(___PRO_DEF_RHS_OP_ACCESSOR, __VA_ARGS__)         \
    };
    ...
    struct accessor<__F, __IsDirect, __D, R(Arg&) Q> {                                             \
        ...
        ___PRO_DEBUG(                                                                              \
            ...
            private : static inline Arg& _symbol_guard(Arg& arg, SELF_ARG)                         \
                NE { return arg __VA_ARGS__ std::forward<decltype(__self)>(__self); })             \
    }
#define ___PRO_ASSIGNMENT_OP_DISPATCH_IMPL(...)                                                    \
    ...
    struct operator_dispatch<#__VA_ARGS__, false> {                                                \
        template<class T, class Arg>                                                               \
        decltype(auto) operator()(T&& self, Arg&& arg) const                                       \
            ___PRO_DIRECT_FUNC_IMPL(std::forward<T>(self) __VA_ARGS__ std::forward<Arg>(arg))      \
                ___PRO_DEF_MEM_ACCESSOR_TEMPLATE(                                                  \
...
struct operator_dispatch<"()", false> {
    ...
    decltype(auto) operator()(T&& self, Args&&... args) const
        ___PRO_DIRECT_FUNC_IMPL(std::forward<T>(self)(std::forward<Args>(args)...))
...
struct implicit_conversion_dispatch : details::cast_dispatch_base<false, false> {
    template<class T>
    T&& operator()(T&& self) const noexcept {
        return std::forward<T>(self);
    }
...
#define ___PRO_EXPAND_MACRO_IMPL(__MACRO, __1, __2, __3, __NAME, ...) __MACRO##_##__NAME
#define ___PRO_EXPAND_MACRO(__MACRO, ...)                                                          \
    ___PRO_EXPAND_IMPL(___PRO_EXPAND_MACRO_IMPL(__MACRO, __VA_ARGS__, 3, 2)(__VA_ARGS__))
...
#define ___PRO_DEF_MEM_DISPATCH_IMPL(__NAME, __FUNC, __FNAME)                                      \
    struct __NAME {                                                                                \
        ...
        decltype(auto) operator()(__T&& __self, __Args&&... __args) const ___PRO_DIRECT_FUNC_IMPL( \
