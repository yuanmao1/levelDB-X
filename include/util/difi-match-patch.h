...
class diff_match_patch {
    ...
    class Diff {
      public:
        ...
        string_t toString() const {
            ...
            return traits::cs(L"Diff(") + strOperation(operation) + traits::cs(L",\"") +
    ...
    class Patch {
      public:
        ...
        string_t toString() const {
            ...
            string_t text(traits::cs(L"@@ -") + coords1 + traits::cs(L" +") + coords2 +
                          traits::cs(L" @@\n"));
  ...
  public:
    diff_match_patch()
        : Diff_Timeout(1.0f), Diff_EditCost(4), Match_Threshold(0.5f), Match_Distance(1000),
          Patch_DeleteThreshold(0.5f), Patch_Margin(4), Match_MaxBits(32) {}
     ...
     */
  private:
    static void diff_main(const string_t& text1,
                          const string_t& text2,
                          bool            checklines,
                          clock_t         deadline,
                          Diffs&          diffs) {
        diffs.clear();
     ...
     */
  private:
    static void diff_compute(
        string_t text1, string_t text2, bool checklines, clock_t deadline, Diffs& diffs) {
        if (text1.empty()) {
  ...
  private:
    static void diff_bisect(const string_t& text1,
                            const string_t& text2,
                            clock_t         deadline,
                            Diffs&          diffs) {
        ...
        std::vector<int> v1(v_length, -1), v2(v_length, -1);
        v1[v_offset + 1] = 0;
        v2[v_offset + 1] = 0;
     ...
     */
  private:
    static void diff_bisectSplit(const string_t& text1,
                                 const string_t& text2,
                                 int             x,
                                 int             y,
                                 clock_t         deadline,
  ...
  protected:
    struct LinePtr : std::pair<typename string_t::const_pointer, size_t> {
        LinePtr() {}
        LinePtr(typename string_t::const_pointer p, size_t n)
            : std::pair<typename string_t::const_pointer, size_t>(p, n) {}
        bool operator<(const LinePtr& p) const {
            return this->second < p.second ? true
    ...
    static void diff_linesToChars(string_t& text1, string_t& text2, Lines& lineArray) {
        ...
        for (typename std::map<LinePtr, size_t>::const_iterator i = lineHash.begin();
             i != lineHash.end(); ++i)
     ...
     */
  private:
    static string_t diff_linesToCharsMunge(const string_t&            text,
                                           std::map<LinePtr, size_t>& lineHash) {
        string_t chars;
        ...
        for (typename string_t::const_pointer lineStart = text.c_str(),
                                              textEnd   = lineStart + text.size();
             lineStart < textEnd; lineStart += lineLen + 1) {
            lineLen = next_token(text, traits::from_wchar(L'\n'), lineStart);
            if (lineStart + lineLen == textEnd)
                --lineLen;
            chars += (char_t)(*lineHash
                                   .insert(std::make_pair(LinePtr(lineStart, lineLen + 1),
                                                          lineHash.size() + 1))
                                   .first)
                         .second;
        }
    ...
    static void diff_charsToLines(Diffs& diffs, const Lines& lineArray) {
        for (typename Diffs::iterator cur_diff = diffs.begin(); cur_diff != diffs.end();
            ...
            for (int y = 0; y < (int)(*cur_diff).text.length(); y++) {
                typedef typename std::make_unsigned<typename string_t::value_type>::type
                               unsigned_value_type;
                const LinePtr& lp = lineArray[static_cast<size_t>(
                    static_cast<unsigned_value_type>((*cur_diff).text[y]))];
  ...
  protected:
    struct HalfMatchResult {
        ...
        void     swap(HalfMatchResult& hm) {
            text1_a.swap(hm.text1_a), text1_b.swap(hm.text1_b), text2_a.swap(hm.text2_a),
                text2_b.swap(hm.text2_b), mid_common.swap(hm.mid_common);
        }
    };
     ...
     */
  private:
    static bool diff_halfMatchI(const string_t&  longtext,
                                const string_t&  shorttext,
                                int              i,
                                HalfMatchResult& best) {
  ...
  public:
    static void diff_cleanupSemanticLossless(Diffs& diffs) {
        ...
        for (typename Diffs::iterator next_diff = cur_diff; ++next_diff != diffs.end();
             prev_diff = cur_diff, cur_diff = next_diff) {
            if ((*prev_diff).operation == EQUAL && (*next_diff).operation == EQUAL) {
    ...
    void diff_cleanupEfficiency(Diffs& diffs) const {
        ...
        for (typename Diffs::iterator cur_diff = diffs.begin(); cur_diff != diffs.end();) {
            if ((*cur_diff).operation == EQUAL) {
                 ...
                 */
                if (!lastequality.empty() && ((pre_ins && pre_del && post_ins && post_del) ||
                        ...
                        if (commonlength != 0) {
                            if (cur_diff != diffs.begin()) {
                                ...
                                if ((*--prev_diff).operation != EQUAL) {
                                    throw string_t(
                                        traits::cs(L"Previous diff should have been an equality."));
                                }
                                (*prev_diff).text += text_insert.substr(0, commonlength);
                            } else {
                                diffs.insert(cur_diff,
                                             Diff(EQUAL, text_insert.substr(0, commonlength)));
                            }
            ...
            for (typename Diffs::iterator next_diff = cur_diff; ++next_diff != diffs.end();
                ...
                if ((*prev_diff).operation == EQUAL && (*next_diff).operation == EQUAL) {
                    ...
                    if ((*cur_diff).text.size() >= (*prev_diff).text.size() &&
                        (*cur_diff).text.compare((*cur_diff).text.size() - (*prev_diff).text.size(),
                                                 (*prev_diff).text.size(),
                                                 (*prev_diff).text) == 0) {
                        ...
                        cur_diff = next_diff;
                        changes  = true;
                        if (++next_diff == diffs.end())
                            break;
                    } else if ((*cur_diff).text.size() >= (*next_diff).text.size() &&
                               (*cur_diff).text.compare(0, (*next_diff).text.size(),
                                                        (*next_diff).text) == 0) {
        ...
        for (typename Diffs::const_iterator cur_diff = diffs.begin(); cur_diff != diffs.end();
            ...
            case DELETE:
                html += traits::cs(L"<del style=\"background:#ffe6e6;\">") + text +
                        traits::cs(L"</del>");
                break;
            case EQUAL: html += traits::cs(L"<span>") + text + traits::cs(L"</span>"); break;
            }
        }
        return html;
    }
  ...
  public:
    static Diffs diff_fromDelta(const string_t& text1, const string_t& delta) {
        ...
        for (typename string_t::const_pointer token = delta.c_str();
             token - delta.c_str() < (int)delta.length(); token += token_len + 1) {
            ...
            default:
                throw string_t(string_t(traits::cs(L"Invalid diff operation in diff_fromDelta: ")) +
                               *token);
            }
        }
  ...
  public:
     ...
     */
  public:
    std::pair<string_t, std::vector<bool>> patch_apply(const Patches&  patches,
                                                       const string_t& text) const {
        std::pair<string_t, std::vector<bool>> res;
        patch_apply(patches, text, res);
        return res;
    }
    void patch_apply(const Patches&                          patches,
                     const string_t&                         sourceText,
        ...
        string_t text1, text2;
        for (typename Patches::const_iterator cur_patch = patchesCopy.begin();
            ...
            if (start_loc == -1) {
                ...
                if (text1 == text2) {
                    ...
                    text = text.substr(0, start_loc) + diff_text2((*cur_patch).diffs) +
                    ...
                    if ((int)text1.length() > Match_MaxBits &&
                        ...
                        diff_cleanupSemanticLossless(diffs);
                        int index1 = 0;
                        for (typename Diffs::const_iterator cur_diff = (*cur_patch).diffs.begin();
                             cur_diff != (*cur_patch).diffs.end(); ++cur_diff) {
                            if ((*cur_diff).operation != EQUAL) {
  ...
  public:
    void patch_splitMax(Patches& patches) const {
        ...
        for (typename Patches::iterator cur_patch = patches.begin(); cur_patch != patches.end();) {
            if ((*cur_patch).length1 <= patch_size) {
                ++cur_patch;
                continue;
            }
            bigpatch = *cur_patch;
            ...
            while (!bigpatch.diffs.empty()) {
                ...
                while (!bigpatch.diffs.empty() && patch.length1 < patch_size - Patch_Margin) {
                    ...
                    if (diff_type == INSERT) {
                        ...
                        diff_text = diff_text.substr(
                            0, std::min((int)diff_text.length(),
                ...
                precontext = safeMid(diff_text2(patch.diffs),
                                     std::max(0, (int)precontext.length() - Patch_Margin));
  ...
  public:
    static string_t patch_toText(const Patches& patches) {
        string_t text;
        for (typename Patches::const_iterator cur_patch = patches.begin();
             cur_patch != patches.end(); ++cur_patch) {
    ...
    Patches patch_fromText(const string_t& textline) const {
        ...
        if (!textline.empty()) {
            ...
            while (text - textline.c_str() < (int)textline.length()) {
                ...
                // A replacement for the regexp "^@@ -(\\d+),?(\\d*) \\+(\\d+),?(\\d*) @@$" exact
                // match
                string_t start1, length1, start2, length2;
                do {
                    typename string_t::const_pointer t = text;
                    l                                  = text_len;
                    if ((l -= 9) > 0 && traits::to_wchar(*t) == L'@' &&
                        traits::to_wchar(*++t) == L'@' && traits::to_wchar(*++t) == L' ' &&
                        traits::to_wchar(*++t) == L'-' && traits::is_digit(*++t)) {
                        do {
                            start1 += *t;
                        } while (--l > 0 && traits::is_digit(*++t));
                        if (l > 0 && traits::to_wchar(*t) == L',')
                            ++t, --l;
                        while (l > 0 && traits::is_digit(*t))
                            --l, length1 += *t++;
                        if (l > 0 && traits::to_wchar(*t++) == L' ' &&
                    ...
                    throw string_t(traits::cs(L"Invalid patch string: ") +
                                   string_t(text, text_len));
                } while (false);
                ...
                for (text += text_len + 1; text - textline.c_str() < (int)textline.length();
                     text += text_len + 1) {
                    if ((text_len = next_token(textline, traits::from_wchar(L'\n'), text)) == 0)
                        continue;
                    ...
                    default:
                        // WTF?
                        throw string_t(traits::cs(L"Invalid patch mode '") +
                                       (sign + (traits::cs(L"' in: ") + line)));
                    }
                    break;
                }
  ...
  private:
    static string_t to_string(int n) {
        ...
        do {
            *--s = traits::from_wchar(digits[n % 10]);
        } while ((n /= 10) > 0);
        if (negative)
            *--s = traits::from_wchar(L'-');
        return str;
    }

    static int to_int(const string_t& str) { return traits::to_int(str.c_str()); }

    static bool is_control(char_t c) {
        switch (traits::to_wchar(c)) {
        case L'\n':
        case L'\r': return true;
        }
        return false;
    }

    static typename string_t::size_type next_token(const string_t&                  str,
                                                   char_t                           delim,
                                                   typename string_t::const_pointer off) {
    ...
    static void append_percent_encoded(string_t& s1, const string_t& s2) {
        const wchar_t safe_chars[] =
            L"0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz-_.~ !*'();/?:@&=+$,#";

        size_t safe[0x100], i;
        for (i = 0; i < 0x100; ++i)
            safe[i] = 0;
        for (i = 0; i < sizeof(safe_chars) / sizeof(wchar_t); ++i)
            safe[safe_chars[i]] = i + 1;
    ...
    static void percent_decode(string_t& str) {
        ...
        for (typename string_t::const_pointer s1 = str.c_str(), end = s1 + str.size(); s1 != end;
             ++s1, ++s2)
        ...
        if (s4 != str.end())
            str.resize(s4 - str.begin());
    }

    static string_t right(const string_t& str, typename string_t::size_type n) {
...
struct diff_match_patch_utf32_from_utf16 {
    ...
    static const unsigned UTF16_SURROGATE_OFFSET =
        (UTF16_SURROGATE_MIN << 10) + UTF16_HIGH_SURROGATE_MAX - 0xFFFFu;
    template<class iterator>
    static iterator to_utf32(iterator i, iterator end, utf32_t& u) {
        u = *i++;
